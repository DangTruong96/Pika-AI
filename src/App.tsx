      ...
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retouch', 'crop', 'adjust', 'filters'] as Tab[]).map(tab => (
                ...
                >
                    {/* Fix: Cast dynamic string to TranslationKey to satisfy TypeScript compiler. */}
                    {t(`tab${tab.charAt(0).toUpperCase() + tab.slice(1)}` as TranslationKey)}
                </button>
            ))}
        </div>
            ...
            {activeTab === 'retouch' && (
                <div className="flex flex-col items-center gap-4">
                    <p className="text-md text-gray-400">
                        {editHotspot ? t('retouchPromptClicked') : t('retouchPrompt')}
                    </p>
                    <form onSubmit={(e) => { e.preventDefault(); handleGenerate(); }} className="w-full flex items-center gap-2">
                        <input
                            ...
                            placeholder={editHotspot ? t('retouchPlaceholderClicked') : t('retouchPlaceholder')}
